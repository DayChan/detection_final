...
def darknet_body(x):
    ...
    x = DarknetConv2D_BN_Leaky(32, (3,3))(x)#320
    x = resblock_body(x, 64, 1)#160
    x = resblock_body(x, 128, 2)#80
    x = resblock_body(x, 256, 8)#40
    x = resblock_body(x, 512, 8)#20
...
def X_DarknetConv2D_BN_Leaky(*args, **kwargs):
    """Darknet Convolution2D followed by BatchNormalization and LeakyReLU."""
    no_bias_kwargs = {'use_bias': False}
    no_bias_kwargs.update(kwargs)
    return compose(
        X_DarknetConv2D(*args, **no_bias_kwargs),
        BatchNormalization(),
        LeakyReLU(alpha=0.1))

def X_resblock_body(x, num_filters, num_blocks):
    '''A series of resblocks starting with a downsampling Convolution2D'''
    # Darknet uses left and top padding instead of 'same' mode
    x = ZeroPadding2D(((1,0),(1,0)))(x)
    x = X_DarknetConv2D_BN_Leaky(num_filters, (3,3), strides=(2,2))(x)
    for i in range(num_blocks):
        y = compose(
                DarknetConv2D_BN_Leaky(num_filters//2, (1,1)),
...
def X_make_last_layers(x, num_filters, out_filters):
    '''6 Conv2D_BN_Leaky layers followed by a Conv2D_linear layer'''
    x = compose(
...
def X_yolo_body(inputs, num_anchors, num_classes):
    """Create YOLO_V3 model CNN body in Keras."""
    darknet = Model(inputs, X_darknet_body(inputs))
    '''
    plot_model(darknet, to_file='model.png',show_shapes=True)
    i=0
    for l in darknet.layers:
...
#新实现dbl=3,res=4+n*7,input刚好为0，
def my_darknet_body(x):
    '''Darknent body having 52 Convolution2D layers'''
    x = DarknetConv2D_BN_Leaky(32, (3,3))(x)
    x = resblock_body(x, 64, 1)
    x = resblock_body(x, 128, 2)
    x = resblock_body(x, 256, 3)#-layer57
    x = resblock_body(x, 512, 3)#-layer82
    x = resblock_body(x, 512, 2)
    return x

def my_make_last_layers(x, num_filters, out_filters):
    '''6 Conv2D_BN_Leaky layers followed by a Conv2D_linear layer'''
    x = compose(
            DarknetConv2D_BN_Leaky(num_filters, (1,1)),
...
def my_yolo_body(inputs, num_anchors, num_classes):
    """Create YOLO_V3 model CNN body in Keras."""
    darknet = Model(inputs, my_darknet_body(inputs))
    x, y1 = my_make_last_layers(darknet.output, 512, num_anchors*(num_classes+5))

    x = compose(
            DarknetConv2D_BN_Leaky(256, (1,1)),
            UpSampling2D(2))(x)
    x = Concatenate()([x,darknet.layers[82].output])
    x, y2 = my_make_last_layers(x, 256, num_anchors*(num_classes+5))

    x = compose(
            DarknetConv2D_BN_Leaky(128, (1,1)),
            UpSampling2D(2))(x)
    x = Concatenate()([x,darknet.layers[57].output])
    x, y3 = my_make_last_layers(x, 128, num_anchors*(num_classes+5))
...
def not_pooling_tiny_yolo_body3(inputs, num_anchors, num_classes):
    x1 = compose(
            DarknetConv2D_BN_Leaky(16, (3,3)),
            X_DarknetConv2D_BN_Leaky(16, (3,3), strides=(2,2),padding='same'),
            DarknetConv2D_BN_Leaky(32, (3,3)),
    ...
    y2 = compose(
            ...
            DarknetConv2D(num_anchors*(num_classes+5), (1,1)))([x2,x1])
    return Model(inputs, [y1,y2])
def not_pooling_tiny_yolo_body4(inputs, num_anchors, num_classes):
    x1 = compose(
            DarknetConv2D_BN_Leaky(16, (3,3)),
            X_DarknetConv2D_BN_Leaky(16, (3,3), strides=(2,2),padding='same'),
            DarknetConv2D_BN_Leaky(32, (3,3)),
    ...
    y2 = compose(
            ...
            DarknetConv2D(num_anchors*(num_classes+5), (1,1)))([x2,x1])
    return Model(inputs, [y1,y2])
def not_pooling_tiny_yolo_body5(inputs, num_anchors, num_classes):
    x1 = compose(
            DarknetConv2D_BN_Leaky(16, (3,3)),
            X_DarknetConv2D_BN_Leaky(16, (3,3), strides=(2,2),padding='same'),
            DarknetConv2D_BN_Leaky(32, (3,3)),
...
def not_pooling_tiny_yolo_body6(inputs, num_anchors, num_classes):
    x1 = compose(
            DarknetConv2D_BN_Leaky(16, (3,3)),
            X_DarknetConv2D_BN_Leaky(16, (3,3), strides=(2,2),padding='same'),
            DarknetConv2D_BN_Leaky(32, (3,3)),
